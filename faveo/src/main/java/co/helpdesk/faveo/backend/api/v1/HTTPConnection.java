...
public class HTTPConnection {
    ...
    String HTTPResponsePost(String stringURL, String parameters) {
        try {
            ...
            Log.e("error en prioridades", e.getMessage());
            e.printStackTrace();
        }
        ...
        try {
            ...
            Log.e("log_tag", "Error convirtiendo resultado " + e.toString());
        }
        if (sb == null)
            return null;
    ...
    public String HTTPResponsePut(String stringURL, String parameters) {
        try {
            ...
            is = connection.getInputStream();
            Log.e("Codigo de Respuesta", connection.getResponseCode() + "");
        } catch (IOException e) {
            if (e.getMessage().contains("No se encontro autentificacion")) {
                if (refreshToken() == null)
                    return null;
                new Helpdesk();
                new Authenticate();
            ...
            Log.e("error en prioridad", e.getMessage());
            e.printStackTrace();
        }
        try {
            BufferedReader reader = new BufferedReader(new InputStreamReader(is, "iso-8859-1"), 8);
    ...
    String HTTPResponseGet(String stringURL) {
        try {
            ...
            if (e.getMessage().contains("No hay autentficacion encontrada")) {
                ...
                Toast.makeText(FaveoApplication.getInstance(), "Oops! Archivo no encontrado", Toast.LENGTH_LONG).show();
            }
